...
The GLMM-based approach as implemented in brms does have known limitations. Most notably, `brms` does not support jointly specifying latent variables within the model formula, which means that certain complex modelling strategies, such as the explanatory multidimensional IRT framework of @kleinsasser2024person where person-level regression models are estimated simultaneously within the IRT likelihood, are not currently specifiable. \rev{This limitation concerns \emph{jointly modelled latent regressions}; correlations among the latent traits themselves are fully supported through the random-effect covariance structure and are used for the correlated-traits model below. Ongoing \texttt{brms} development is expanding its latent-variable functionality.}[^brms3] Nevertheless, for the class of models covered in this tutorial, `brms` offers a compelling combination of one-stop integration and concise, transferable syntax. Within a single formula interface, researchers can specify multiple ordinal model families (cumulative and adjacent-category links), multidimensional structures (correlated-traits and bi-factor), item-specific discrimination via distributional regression, covariates, and differential item functioning, all with built-in model comparison through approximate leave-one-out cross-validation [@vehtari2017practia].
...
\rev{Classical item-fit statistics (e.g., infit, outfit, $S$-$X^2$) originate in the Rasch tradition, but they are not inherently frequentist: they also have posterior-predictive Bayesian forms.} @bohn2025previc \rev{compute such statistics for each posterior draw and aggregate them, and the \texttt{easyRaschBayes} package} [@johansson2026easyra] \rev{implements posterior-predictive infit/outfit directly for \texttt{brms} models, summarising item fit through posterior predictive $p$-values (ppp). These are a special case of the broader posterior predictive checking (ppc) approach to item-level diagnostics} [@winter2025tutoria]. \rev{We use posterior-predictive infit/outfit as a complementary cross-check when freeing discrimination (Step 3).}
...
\rev{The same conclusion can also be reached through item-fit diagnostics. Applying the Bayesian posterior-predictive infit and outfit, which are closer to the Rasch community's intuition, to the RSM (}\texttt{merge\_rsm}\rev{) under conventional cutoffs returns similar information across the full item set: Bm\_1\_r, Bm\_2\_r, and Bm\_4\_r are again flagged as unacceptable, while Bm\_3\_r survives as acceptable.} \rev{For brevity,} @tbl-itemfit \rev{reports only these four items.}

| Item | Infit MNSQ | Infit *p* | Outfit MNSQ | Outfit *p* |
|:--|--:|--:|--:|--:|
| Bm_1_r | 1.34 | 0.018 | 1.29 | 0.070 |
| Bm_2_r | 1.64 | 0.004 | 1.48 | 0.046 |
| Bm_3_r | 1.09 | 0.230 | 1.17 | 0.140 |
| Bm_4_r | 1.38 | 0.002 | 1.33 | 0.014 |

: Posterior-predictive infit and outfit (mean-square and one-sided posterior predictive *p*) for the four reverse-coded mathematics items, from the full-instrument RSM (`merge_rsm`); the remaining items showed no item-fit anomaly. Under conventional cutoffs (infit mean-square > 1.3, infit *p* < 0.05), Bm_1_r, Bm_2_r, and Bm_4_r are unacceptable while Bm_3_r is within bounds; outfit is reported for reference. Estimates use 2,000 posterior draws. {#tbl-itemfit}
...
| General | 0.61 | [0.50, 0.71] |
| Mathematics | 0.62 | [0.53, 0.71] |
| Learners | 0.55 | [0.44, 0.66] |
| Pedagogy | 0.32 | [0.14, 0.49] |
...
\rev{In this notation the term after the vertical bar is the grouping factor: grouping on item gives each item its own gender (and district) effect, partially pooled across items, which is what a DIF test requires. The cell-means coding }\texttt{(0 + gender | item)}\rev{ and the equivalent }\texttt{(1 + gender | item)}\rev{ re-express the same bivariate-normal model in two forms: separate male and female levels per item, or a reference level plus a male-female difference.}

\rev{The benefit of the cell-means coding} is that we can directly inspect how males and females perform on each item. However, determining whether the male-female difference is statistically meaningful is less straightforward, because we do not have a single posterior for the difference itself. @bohn2025previc \rev{offers a practical, diagnostic heuristic: flagging an item when its male and female 95\% credible intervals do not overlap, that is, when one interval lies entirely above or below the other. None of the 20 items is flagged on this criterion (Panel A of} @fig-dif\rev{). This is a deliberately conservative screen rather than a formal DIF test: requiring two 95\% credible intervals to be disjoint is a stronger condition than the difference itself being credible, so genuine but smaller differences can be missed.}[^8]
