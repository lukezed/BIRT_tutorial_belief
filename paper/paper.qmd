...
We'll briefly introduce the ordinal IRT models and dimensional structures used in this tutorial, alongside the existing work that informs our approach. 
 
The most constrained yet concise ordinal IRT model considered used in this paper is the rating scale model [RSM, @andrich1978rating], which uses an adjacent-category logit link and assumes that all items share both the same set of thresholds and the same discrimination ($\alpha = 1$). Relaxing the threshold constraint so that each item has its own threshold spacing yields the partial credit model [PCM, @masters1982rasch]. Both the RSM and PCM constrain discrimination to be equal across items. Introducing item-specific discrimination produces their generalised counterparts: the GRSM and GPCM, respectively. Another widely used model that simultaneously estimates item-specific difficulties and discrimination is the graded response model [GRM, @samejima1969estima], which differs from the adjacent-category family in its use of a cumulative logit link: rather than modelling the odds of each category relative to its neighbour, the GRM models the cumulative probability $P(Y \geq c)$ of responding at or above each threshold.[^grm-note] @burkner2019ordina provide a detailed treatment of the distinction between adjacent-category and cumulative link functions, demonstrating that the two families can produce substantively different parameter estimates and model fit for the same data. @burkner2021bayesi further shows how all of these models can be specified in `brms` using its distributional regression syntax, where discrimination is introduced via `disc ~ 1 + (1 | item)` and the link function is controlled through the family argument. Importantly, freeing discrimination is not merely a matter of model flexibility: as we demonstrate in the workflow, it also serves a diagnostic purpose, revealing which items contribute meaningful information and whether threshold anomalies arise from model constraints rather than data problems.
 
[^grm-note]: In its original formulation [@samejima1969estima], the GRM estimates item-specific thresholds. The implementation used in this tutorial is a constrained variant in which threshold spacing is shared across items, with item differences captured through a location parameter that shifts all thresholds equally. See @apx-models for details.
...
The orthogonality constraint $\text{Cov}(\theta_p, \theta_{pd}^{s}) = 0$ is imposed here.[^1] Competing combinations are systematically compared in the workflow later (also see @apx-models for modelling details).
...
[^2]: The LKJ prior [@lewandowski2009genera] is a distribution over correlation matrices. We use $\eta = 2$ as a mild regularising choice.
...
Before collapsing, we can first fitted a baseline Rating Scale Model (RSM) to the original 5-point data to inspect the threshold structure. In `brms`, the RSM is specified using `brmsfamily("acat", "logit")`, where `acat` denotes the adjacent-category parameterisation (modelling the odds of each response category relative to its neighbour) and `logit` specifies the logistic link function.
...
| Threshold Transition | `bf_rsm` (Est. \[95% CrI\]) | `ct_rsm` (Est. \[95% CrI\]) |
|:-----------------------|:-----------------------|:-----------------------|
| 1: "disagree" to "neutral" | $-0.55 \ [-1.10, 0.01]$ | $-0.54 \ [-1.11, 0.05]$ |
| 2: "neutral" to "agree" | $-0.55 \ [-1.10, 0.01]$ | $-0.54 \ [-1.11, 0.04]$ |
| 3: "agree" to "strongly agree" | $2.44 \ [1.88, 3.00]$ | $2.46 \ [1.89, 3.04]$ |

: Threshold estimates from the multidimensional RSMs. {#tbl-thresh-dim tbl-colwidths="false"}
...
For `bf_grm_20`, moment matching successfully resolved the single problematic $\hat{k}$ value. If moment matching fails to resolve all problematic observations, `reloo = TRUE` can be added, which refits the entire model once for each remaining problematic observation.[^3]

[^3]: Each refit runs the full MCMC sampling procedure. In our case, each refit took approximately 30 minutes, making 2 refits manageable.
...
[^4]: This is analogous to a situation in frequentist inference where two confidence intervals overlap but the difference between the two means can be nonetheless statistically significant.
...
![Overview of the Implemented Bayesian IRT Workflow (adapted from Gelman et al., 2020). Note: Dashed boxes indicate steps completed in the full workflow but not explicitly presented in this paper. Yellow boxes represent decision-making points. Dashed arrows connect model modification solutions to their corresponding model comparison steps. Bolded model names indicate those that outperformed others in preliminary model comparisons.](../figures/fig8_workflow.png){#fig-workflow width="95%"}
 
The workflow has also demonstrated adaptability beyond the present case. Using a similar framework, we validated a separate instrument measuring mathematics teachers' pedagogical practice [@zhang2025Belief]. That analysis followed a different path: when moving from the RSM to the PCM to free item-specific thresholds, the diagnostics revealed that all reverse-coded items exhibited abnormal threshold patterns, suggesting that the forced shared-threshold structure of the RSM had been masking item-level problems. These items were originally designed to capture a transmissionist orientation assumed to be the conceptual opposite of the instrument's connectionist focus, but their disordered thresholds suggest that this binary opposition may not hold in the sampled population. That the same workflow framework led to different outcomes and different substantive conclusions for a different instrument supports its value as a decision framework rather than a fixed recipe.
 
It is important to note that a comprehensive Bayesian workflow can encompass more than what we have demonstrated here. Prior predictive checks and prior sensitivity analysis, as demonstrated by @winter2025tutoria, were not incorporated. This reflects a deliberate scope decision: our focus was on model comparison and diagnostic reasoning as drivers of iterative model selection, and prior specification was treated as a relatively settled choice following @burkner2021bayesi. Researchers working in contexts where prior sensitivity is a particular concern should integrate these additional steps.
