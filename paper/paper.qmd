...
\rev{As a complementary cue for model selection, we also report the LOO-based stacking weights} [@yao2018using]\rev{: a model that receives almost all of the weight corroborates its selection. We use these weights only as supporting evidence alongside the ELPD differences, not to form a combined model.}
...
With the four misfitting items removed, thresholds under both `ct_grsm_20` and `bf_grsm_20` are now in monotonic order as showed in @tbl-grsm-20-summary. `ct_grsm_20` estimates moderate to strong correlations among the three dimensions, with the strongest association between mathematics and pedagogy beliefs ($r = 0.51$, 95% CrI $[0.31, 0.68]$), suggesting that teachers who view mathematics as a creative and applied discipline also tend to be more receptive to innovative teaching approaches. `bf_grsm_20` also successfully separates a general inclusive belief factor (SD $= 1.73$, 95% CrI $[0.82, 2.98]$) from dimension-specific residuals.
