...
|:--|--:|--:|--:|--:|
| Bm_1_r | 1.34 | .018 | 1.29 | .070 |
| Bm_2_r | 1.64 | .004 | 1.48 | .046 |
| Bm_3_r | 1.09 | .230 | 1.17 | .140 |
| Bm_4_r | 1.38 | .002 | 1.33 | .014 |

: Posterior-predictive infit and outfit (mean-square and one-sided posterior predictive *p*) for the four reverse-coded mathematics items, from the full-instrument RSM (`merge_rsm`); the remaining items showed no item-fit anomaly. Under conventional cutoffs (infit mean-square > 1.3, infit *p* < .05), Bm_1_r, Bm_2_r, and Bm_4_r are unacceptable while Bm_3_r is within bounds; outfit is reported for reference. Estimates use 2,000 posterior draws. {#tbl-itemfit}
...
\rev{The reliability of the final model supports the bi-factor specification (}@tbl-reliability\rev{). The total score is highly reliable ($\omega_{\text{total}} = .90$), and most of that reliable variance is carried by the general factor ($\omega_{\text{H}} = .61$, ECV $= .68$). This is precisely the pattern a bi-factor structure implies: the instrument measures a single inclusive-belief construct well, while the domain-specific factors contribute limited reliable variance beyond it. The per-dimension reliabilities follow the same ordering, which is why the instrument is properly scored and interpreted as a whole rather than as independent subscales.}

| Quantity | Estimate | 95% interval |
|:--|--:|:--:|
| **Total-score reliability** | | |
| $\omega_{\text{total}}$ | .90 | [.87, .92] |
| $\omega_{\mathrm{H}}$ (general factor) | .61 | [.48, .72] |
| ECV (general factor) | .68 | [.55, .79] |
| **Per-dimension reliability (RMU)** | | |
| General | .61 | [.50, .71] |
| Mathematics | .62 | [.53, .71] |
| Learners | .55 | [.44, .66] |
| Pedagogy | .32 | [.14, .49] |
...
\rev{In this notation the term after the vertical bar is the grouping factor: by grouping on item, each item carries its own gender (and district) effect that is partially pooled across items, which is what a DIF test requires. The two codings we use differ only in form, not in substance: }\texttt{(0 + gender | item)}\rev{ estimates a separate male and female location per item (jointly bivariate normal), whereas }\texttt{(1 + gender | item)}\rev{ re-expresses the same model as a reference level plus a male-female difference.}
