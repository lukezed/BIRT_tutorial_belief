...
The GLMM-based approach as implemented in brms does have known limitations. Most notably, `brms` does not support jointly specifying latent variables within the model formula, which means that certain complex modelling strategies, such as the explanatory multidimensional IRT framework of @kleinsasser2024person where person-level regression models are estimated simultaneously within the IRT likelihood, are not currently specifiable. \rev{This limitation concerns \emph{jointly modelled latent regressions}: correlations among the latent traits themselves are supported through the random-effect covariance structure, as we use for the correlated-traits model below.}[^brms3] Nevertheless, for the class of models covered in this tutorial, `brms` offers a compelling combination of one-stop integration and concise, transferable syntax. Within a single formula interface, researchers can specify multiple ordinal model families (cumulative and adjacent-category links), multidimensional structures (correlated-traits and bi-factor), item-specific discrimination via distributional regression, covariates, and differential item functioning, all with built-in model comparison through approximate leave-one-out cross-validation [@vehtari2017practia].

[^brms3]: \rev{See, e.g., the planned latent-variable features discussed at} \url{https://github.com/paul-buerkner/brms/issues/304}.
...
\rev{As a complementary cue for model selection, we also report the LOO-based stacking weights} [@yao2018using]\rev{: a model that receives almost all of the weight corroborates its selection.}
...
\rev{Item-fit statistics (e.g., infit, outfit, $S$-$X^2$) are not inherently frequentist: they also have posterior-predictive Bayesian forms.} @bohn2025previc \rev{compute such statistics for each posterior draw and aggregate them, and the \texttt{easyRaschBayes} package} [@johansson2026easyra] \rev{implements posterior-predictive infit/outfit directly for \texttt{brms} models, summarising item fit through posterior predictive $p$-values (ppp). These are a special case of the broader posterior predictive checking (ppc) approach to item-level diagnostics} [@winter2025tutoria]. \rev{We treat an item as underfitting when its infit mean-square exceeds 1.3 and, equivalently, its posterior predictive $p$ falls below 0.05, both signalling more response noise than the model predicts; the opposite tail (a low mean-square, $p$ near 1) reflects overly predictable responses and is not treated as misfit. We use these statistics as a complementary cross-check when freeing discrimination (Step 3).}


## \rev{Reliability}
\rev{For the final model we summarise measurement precision with reliability coefficients. Because the instrument is multidimensional, no single latent dimension is meant to carry the measurement alone, so we report two complementary views. Model-based omega coefficients} [@rodriguez2016evalua; @mcdonald2013test] \rev{describe the total score: $\omega_{\text{total}}$ is the proportion of total-score variance that is true-score (common) variance, while $\omega_{\text{H}}$ and the explained common variance (ECV) quantify how much of that is attributable to the general factor. Separately, the per-dimension reliability of the person estimates, computed through the relative measurement uncertainty (RMU) of} @bignardi2025genera\rev{, indexes how precisely individuals can be placed on each latent dimension; under partial pooling at small samples these are conservative lower bounds. Together they separate the reliability of the instrument as a whole from the precision of any single subscale. All coefficients are computed over the full posterior, yielding credible intervals.}
...
![Item-level response distributions across the original 5-point scale, coloured by subscale dimension. \rev{Item codes: Bm = beliefs about mathematics, Bl = beliefs about learners, Bp = beliefs about pedagogy; the suffix \_r marks reverse-coded items (full wording in Appendix B).}](../figures/fig3_response_distribution.png){#fig-response-dist width="95%"}
...
![Wright map from `merge_rsm`. Left: item difficulty estimates with 95% CrIs. Right: person ability estimates sorted by magnitude. \rev{Item codes: Bm = beliefs about mathematics, Bl = beliefs about learners, Bp = beliefs about pedagogy; the suffix \_r marks reverse-coded items (full wording in Appendix B).}](../figures/fig4_wright_map.png){#fig-wright-map-rsm width="95%"}
...
![Item difficulty (left) and discrimination (right) estimates from the `ct_grsm` (top) and `bf_grsm` (bottom). Discrimination values are shown on the exponentiated scale ($e^\alpha$). The dashed line at 0.5 marks a conventional threshold below which items are considered poor discriminators. \rev{Item codes: Bm = beliefs about mathematics, Bl = beliefs about learners, Bp = beliefs about pedagogy; the suffix \_r marks reverse-coded items (full wording in Appendix B).}](../figures/fig5_model_comparison.png){#fig-wright-grsm width="95%"}

The discrimination panels unsurprisingly reveal substantially different discrimination values across items, confirming that the equal-discrimination assumption was inappropriate. Most items cluster in a reasonable range, but four items stand out: Bm_1_r, Bm_2_r, Bm_3_r, and Bm_4_r. These four items all belong to the "beliefs about mathematics" subscale and share the same reverse-coded formalist orientation (e.g., "Mathematics is a collection of rules and procedures"). Their discrimination estimates fall well below the conventional $\alpha = 0.5$ threshold for 'poor' items [@embretson2013itema]. This indicates that item responses are virtually independent of the measured latent trait: a respondent's location on the latent continuum exerts little to no influence on their probability of selecting a given category. Their difficulty estimates are simultaneously problematic with very wide CrIs, meaning they are poorly identified by the data and nearly all respondents find it easy to endorse the exclusive end. This pattern was invisible under the RSM, because the equal-discrimination assumption diluted these poor measurement properties across the shared parameter structure rather than localising them to specific items.

\rev{Our removal of all four items rests on the freed-discrimination analysis above. As an independent cross-check, the Bayesian posterior-predictive infit and outfit introduced earlier, computed on the RSM (}\texttt{merge\_rsm}\rev{), flag three of the same four items (}@tbl-itemfit\rev{): Bm\_1\_r, Bm\_2\_r, and Bm\_4\_r exceed the underfit cutoffs (infit mean-square above 1.3, posterior predictive p below 0.05) and stand apart from the rest by a clear gap. Bm\_3\_r is the one point of difference: it passes item fit and would be retained under a pure Rasch screen, and is exposed only once discrimination is freed. We therefore take the discrimination evidence as decisive and remove all four.}

| Item | Infit MNSQ | Infit *ppp* | Outfit MNSQ | Outfit *ppp* |
|:--|--:|--:|--:|--:|
| **Bm_2_r** | $\mathbf{1.64}$ | $\mathbf{0.022}$ | $\mathbf{1.48}$ | $\mathbf{0.115}$ |
| **Bm_4_r** | $\mathbf{1.38}$ | $\mathbf{0.002}$ | $\mathbf{1.33}$ | $\mathbf{0.025}$ |
| **Bm_1_r** | $\mathbf{1.34}$ | $\mathbf{0.039}$ | $\mathbf{1.29}$ | $\mathbf{0.126}$ |
| Bm_3_r | $1.09$ | $0.316$ | $1.17$ | $0.238$ |
| Bl_11_r | $1.09$ | $0.174$ | $1.08$ | $0.213$ |
| Bl_6_r | $1.09$ | $0.190$ | $1.06$ | $0.312$ |
| Bl_10_r | $1.09$ | $0.186$ | $1.08$ | $0.216$ |
| Bl_9_r | $1.08$ | $0.207$ | $1.07$ | $0.238$ |
| Bl_2 | $1.07$ | $0.315$ | $1.17$ | $0.133$ |
| Bp_2_r | $1.02$ | $0.434$ | $1.00$ | $0.501$ |
| Bm_5 | $1.02$ | $0.406$ | $1.05$ | $0.337$ |
| Bm_8 | $1.01$ | $0.459$ | $1.06$ | $0.344$ |
| Bl_7_r | $1.00$ | $0.494$ | $1.01$ | $0.446$ |
| Bl_8_r | $0.98$ | $0.570$ | $0.99$ | $0.556$ |
| Bp_1_r | $0.97$ | $0.608$ | $0.98$ | $0.546$ |
| Bl_5 | $0.94$ | $0.650$ | $0.99$ | $0.504$ |
| Bl_3_r | $0.94$ | $0.716$ | $0.93$ | $0.739$ |
| Bm_6 | $0.89$ | $0.786$ | $0.93$ | $0.680$ |
| Bp_5 | $0.88$ | $0.795$ | $0.94$ | $0.638$ |
| Bm_7 | $0.82$ | $0.909$ | $0.87$ | $0.817$ |
| Bl_4_r | $0.81$ | $0.981$ | $0.81$ | $0.968$ |
| Bl_1_r | $0.80$ | $0.980$ | $0.81$ | $0.955$ |
| Bp_4 | $0.67$ | $0.992$ | $0.74$ | $0.973$ |
| Bp_3 | $0.62$ | $0.999$ | $0.64$ | $0.996$ |

: Posterior-predictive infit and outfit for all 24 items at the RSM stage (`merge_rsm`), ordered by infit. An item is flagged as underfitting when its infit mean-square exceeds 1.3 and its posterior predictive *p* falls below 0.05 (both indicating more response noise than the model predicts): Bm_1_r, Bm_2_r, and Bm_4_r meet this and are separated from the rest by a clear gap. The opposite tail (low mean-square, *p* near 1, e.g. Bp_3) reflects overly predictable responses and is not treated as misfit. The *p*-values are Monte Carlo posterior-predictive estimates and vary slightly across runs, although the resulting classification is stable. {#tbl-itemfit}
...
With the four misfitting items removed, thresholds under both `ct_grsm_20` and `bf_grsm_20` are now in monotonic order as shown in @tbl-grsm-20-summary. `ct_grsm_20` estimates moderate to strong correlations among the three dimensions, with the strongest association between mathematics and pedagogy beliefs ($r = 0.51$, 95% CrI $[0.31, 0.68]$), suggesting that teachers who view mathematics as a creative and applied discipline also tend to be more receptive to innovative teaching approaches. `bf_grsm_20` also successfully separates a general inclusive belief factor (SD $= 1.73$, 95% CrI $[0.82, 2.98]$) from dimension-specific residuals.
...
Crossing these three link function families with two dimensional structures (CT and BF) yields six candidate models, all fitted to the same 20-item, 4-category dataset. @tbl-loo-final-compare shows the comparison results.
...
![Diagnostic Wright map for the final model (`bf_grm_20`). Top left: item difficulty with 95% CrIs. Top right: item discrimination (exponentiated scale). Bottom panels: person parameter estimates for the general factor and three dimension-specific factors, sorted by magnitude. \rev{Item codes: Bm = beliefs about mathematics, Bl = beliefs about learners, Bp = beliefs about pedagogy; the suffix \_r marks reverse-coded items (full wording in Appendix B).}](../figures/fig6_final_diagnostics.png){#fig-final-diag width="95%"}

The item difficulty panel shows a well-distributed spread across the latent continuum with no abnormally wide credible intervals, confirming that the removal of the four misfitting items resolved the identification problems observed in Step 3. Discrimination values for the retained items are all above the $\alpha = 0.5$ threshold. The person parameter panels show approximately normal distributions for all four factors, with the general factor representing the inclusive orientation shared across all items.
...
: Reliability of the final model (`bf_grm_20`), computed over the full posterior. Total-score coefficients are model-based omega [@rodriguez2016evalua]; per-dimension entries are the relative measurement uncertainty (RMU) reliability with its 95% HDCI. {#tbl-reliability}
...
One intuitive approach is to specify `(0 + gender | item)` alongside `gender` as a fixed effect:
...
\rev{In this notation the variable after the vertical bar is the grouping factor; here it is }\texttt{item}\rev{, so each item carries its own male and female level (jointly bivariate normal). The benefit of }\texttt{(0 + gender | item)} \rev{ is that we can read the two group posteriors directly. Following} @bohn2025previc\rev{, Panel A of} @fig-dif \rev{flags an item when its male and female 95\% credible intervals do not overlap, and none of the 20 items is. This heuristic is convenient but not strict: overlapping intervals can still hide a credible difference.}[^8]
...
@fig-dif \rev{presents the full DIF analysis, and the conservative overlap screen (Panel A) and the stricter difference test agree. In Panel B every item's gender DIF effect (log-odds, male minus female, from }\texttt{bf\_grm\_dif2} \rev{) has a 95\% CrI that includes zero, as does every district effect in Panel C (Districts 2 and 3 relative to District 1). The instrument thus appears to function consistently across gender and district subgroups.}

![Differential item functioning and covariate analysis. Panel A: posterior overlap diagnostic for gender `bf_grm_dif`. Panel B: item-level gender DIF effects from `bf_grm_dif2`. Panel C: item-level district DIF effects. Panel D: Estimated category response probabilities by district. \rev{Item codes: Bm = beliefs about mathematics, Bl = beliefs about learners, Bp = beliefs about pedagogy; the suffix \_r marks reverse-coded items (full wording in Appendix B).}](../figures/fig7_combined_dif.png){#fig-dif width="95%"}
...
## brms code for all fitted models

For quick reference, the listing below gives the brms specification of every model fitted in this study. It is deliberately minimal: the `bf()` model formulas, then the `brm()` calls whose `family` and `data` arguments set the models apart (the `family` chooses the link / threshold form, as in @tbl-models). Priors are weakly informative as in @tbl-priors; computational settings (chains, iterations, `adapt_delta`, backend, etc.) are omitted. Complete runnable code is in the companion repository.

\small

```r
# Model formulas (random effects encode the dimensional structure)

# Unidimensional
bf_uni <- bf(response ~ 1 + (1 | item) + (1 | id))

# Correlated-traits MIRT (three correlated dimensions)
bf_ct <- bf(response ~ 1 + (1 | item) + (0 + dim_m + dim_l + dim_p | id))

# Bi-factor MIRT (general factor + orthogonal specific factors)
bf_bf <- bf(
  response ~ 1 + (1 | item) + (1 || id) +
    (0 + dim_m || id) + (0 + dim_l || id) + (0 + dim_p || id))

# 2PL variants add an item-specific discrimination submodel
bf_ct_2pl <- bf(
...
# GPCM variants use item-specific thresholds via cs(1)
bf_ct_gpcm <- bf(
  response ~ 1 + (cs(1) | item) + (0 + dim_m + dim_l + dim_p | id),
  disc ~ 1 + (1 | item))
bf_bf_gpcm <- bf(
  response ~ 1 + (cs(1) | item) + (1 | id) +
    (0 + dim_m || id) + (0 + dim_l || id) + (0 + dim_p || id),
  disc ~ 1 + (1 | item))

# DIF models add group covariates with item-level random slopes
bf_dif <- bf(
  response ~ 1 + gender + (0 + gender | item) + (1 | id) +
    (0 + dim_m || id) + (0 + dim_l || id) + (0 + dim_p || id),
  disc ~ 1 + (1 | item))
bf_dif2 <- bf(
  response ~ 1 + gender + District + (1 + gender + District || item) +
    (1 | id) + (0 + dim_m || id) + (0 + dim_l || id) + (0 + dim_p || id),
  disc ~ 1 + (1 | item))

# Fitted models. acat = adjacent-category, cuml = cumulative logit.
#   RSM  = acat, no disc        GRSM = acat + disc
#   GPCM = acat + cs() + disc   GRM  = cuml + disc
acat <- brmsfamily("acat", "logit")
cuml <- brmsfamily("cumulative", "logit")

null_rsm    <- brm(bf_uni,     family = acat, data = data_1) # 5-category
merge_rsm   <- brm(bf_uni,     family = acat, data = data_2) # 4-category
ct_rsm      <- brm(bf_ct,      family = acat, data = data_2)
bf_rsm      <- brm(bf_bf,      family = acat, data = data_2)
ct_grsm     <- brm(bf_ct_2pl,  family = acat, data = data_2)
bf_grsm     <- brm(bf_bf_2pl,  family = acat, data = data_2)
ct_grsm_20  <- brm(bf_ct_2pl,  family = acat, data = data_3) # 4 removed
bf_grsm_20  <- brm(bf_bf_2pl,  family = acat, data = data_3)
ct_gpcm_20  <- brm(bf_ct_gpcm, family = acat, data = data_3)
bf_gpcm_20  <- brm(bf_bf_gpcm, family = acat, data = data_3)
ct_grm_20   <- brm(bf_ct_2pl,  family = cuml, data = data_3)
bf_grm_20   <- brm(bf_bf_2pl,  family = cuml, data = data_3) # final
bf_grm_dif  <- brm(bf_dif,     family = cuml, data = data_3)
bf_grm_dif2 <- brm(bf_dif2,    family = cuml, data = data_3)
```
