...
{\caption{{Threshold estimates from the baseline RSM fitted to the
original 5-point response scale.}{\label{tbl-thresh-null}}}
\vspace{-20pt}}
...
The threshold reversal however persists. Thresholds 1 and 2 of the
collapsed scale remain in reversed order with heavily overlapping
credible intervals, confirming that the anomaly was not attributable to
category sparsity alone. The RSM constrains all items to share the same
set of thresholds; when items differ substantially in how well they
discriminate between respondents, this shared structure is forced into a
compromise that can compress or reverse threshold spacing. The
persisting reversal therefore points in two directions: the underlying
construct may have a more complex dimensionality than the unidimensional
model assumes (Step 2), and some items may not discriminate well enough
under a constrained equal-discrimination specification (Step 3).
